...
describe("Google Generative AI provider stream events", () => {
	it("forwards each SDK chunk in order before normalizing it", async () => {
		...
		const result = await streamGoogleGenerativeAi(model, context, {
			apiKey: "test-api-key",
			onProviderStreamEvent: async (chunk, eventModel) => {
