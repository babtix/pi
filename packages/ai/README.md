...
`createProvider()` handles dynamic publication and persistence automatically. Handwritten `Provider.refreshModels()` implementations receive the read-only `context.stored` snapshot and publish through `context.publish({ persist?, update? })`. Omit `persist` to leave storage unchanged, pass a `ModelsStoreEntry` to write it, or pass `persist: null` to delete it. A `ModelsStoreEntry` keeps chat models in `models` and every other model type in `otherModels`, so readers that predate other model types only see chat models. Publication is generation-checked; put synchronous in-memory catalog changes in `update` rather than mutating state before publication.
